import pygame
import random
from functions import load_image, play_music, terminate, scoreupdate, high_score #часть загрузочных функций перенес в отдельный файл для чистоты кода
velocity = 500
bvelocity = 700 #скорость птицы
...
isJump = False
JumpCount = 10
#создаем группы спрайтов для кнопок в каждом окне (для каждого окна - своя группа)
...
class Board:
    ...
    #поскольку размер клетки - 10, то соответсвенно 40 * 10 = 400(с 400 по y коориднате начинается поле)
...
def game(velocity, bvelocity, color, isJump, JumpCount, running, name):
    ...
    board.render(color)
    lst_cactus = [] #создаем списки наших спрайтов (на деле стоило использовать группы)
    lst_bird = []
    scorelist = []
    ...
    while running:
        ...
        plantchoose = random.randint(1, 3) #рандомный выбор будущей птицы/растения
        screen.fill((255, 255, 255))
        high_score(screen)
        ...
        if isJump:
            if JumpCount <= 10:
                dino.rect.y += int(JumpCount* 10/2**3)   #физка прыжка, за основу взято земное ускорение и формула (вскоре переработанная)
            elif JumpCount >= -10:
                isJump = False
            JumpCount += 1/2
        if time >= 10:
